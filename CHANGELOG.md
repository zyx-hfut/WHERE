...
- 新增轻量知识图谱页面。
- 每个列表可以独立生成物品节点、位置节点和“放在”关系边。
- 支持从侧边栏或物品页面进入知识图谱，并切换不同列表查看。

### Fixed

- 更新检查增加 Tauri updater 权限配置和 30 秒超时。
- 更新失败提示包含 GitHub `latest.json`、网络、权限或签名错误的具体信息。

## [0.14.3-runtime-fix] - 2026-09-15

### Fixed

- 将 Windows 隐藏控制台配置补充到真正的可执行入口，正式安装包不再额外弹出黑色控制台窗口。
- 修正 Tauri 配置版本号与发布版本不一致的问题。
- 开发模式使用独立应用标识，避免调试账号与正式安装版共享本地数据库。
- 发布流程显式上传 `latest.json`，并优先使用 NSIS 更新包。
...
- 桌面端备份现在会由 Rust 直接写入系统“下载”目录，不再依赖 WebView 的下载行为。
- 恢复备份时允许附件缺少二进制字节，以兼容浏览器端导出的备份；桌面端自身导出的图片仍会完整恢复。
- 恢复后刷新列表和当前物品数据，减少界面显示旧数据造成的误判。
- 原子操作标志强制使用白色文字，提高在不同主题下的可读性。

### Known limitations

- PC/手机安全同步、冲突解决、实时推送尚未实现。
- 加密备份、导入预览、撤销恢复尚未实现。
- 在线更新、移动端发布流水线和原生文件选择器仍待完善。
...
- 增加 `create_list` 列表原子工具。
- 新建列表可以被后续 `create_item` 步骤通过 `list_ref` 引用。
- 列表创建和物品新增会在同一份待确认计划中展示。
- 物品页面支持按位置分组、按物品分组和平铺三种展示方式。
- 位置和物品分组均支持折叠/展开。
- 同名物品在“按物品”模式下聚合展示，保留各条记录信息。
- 增加密码列表和密码物品绑定测试。

### Fixed

- 修复模型将“密码列表”误当作物品位置的问题。
- 修复密码值被错误拆成备注的 Mock 场景；密码平台信息按独立列表和物品记录规划。
...
## [0.11.1-action-preview] - 2026-09-10
...
## [0.11.0-agent-vector-search] - 2026-09-10

### Changed

- 移除 Agent 名称、位置和备注检索中的手写字符串匹配规则。
- 新增轻量字符 n-gram 向量化和余弦相似度模块。
- 名称、位置、备注搜索使用相似度排序和阈值筛选。
- 支持自然语言位置差异，例如“床头柜第一个抽屉”和“床头柜的第一个抽屉”。
- 语义类别查询保留模型候选判断，不把“吹风机”等物品硬编码为唯一类别答案。
- 增加向量相似度单元测试。
